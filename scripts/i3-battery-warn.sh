#!/bin/bash
SLEEP_TIME=1   # Default time between checks.
SAFE_PERCENT=20  # Still safe at this level.
DANGER_PERCENT=7  # Warn when battery at this level.
CRITICAL_PERCENT=5  # Hibernate when battery at this level.
NAGBAR_FONT="pango:DejaVu Sans Mono 16"
...
{
    i3-nagbar -f "$NAGBAR_FONT" -m 'Battery low!' -b 'Hibernate!' 'pm-hibernate' >/dev/null 2>&1 &
    NAGBAR_PID=$!
}
